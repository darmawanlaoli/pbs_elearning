...
use App\Exports\Admin\PrimaryAssesmentRecordExport as AdminPrimaryAssesmentRecordExport;
use App\Exports\Admin\MandarinPrimaryAssesmentRecordExport as AdminMandarinPrimaryAssesmentRecordExport;
...
class AssesmentRecord extends Controller
{
    public function index()
    {
        ...
        $assesments = DB::table('primary_assesment_records')
            ...
            ->orderBy('class', 'ASC')
            ->get();
        return view('adminprimary.assesment_record.index', compact('title', 'path', 'assesments'));
    }
    ...
    public function detail($id)
    {
        ...
        if($subject->subject == 'HEALTH AND PHYSICAL EDUCATION') {
            return view('adminprimary/assesment_record/edit_pe', compact('title', 'path', 'assesments', 'subject'));
        }elseif($subject->subject == 'ENGLISH' || $subject->subject == 'BAHASA INDONESIA') {
            if(in_array($grade, ['P1', 'P2', 'P3'])) {
                return view('adminprimary/assesment_record/edit_lang_lower', compact('title', 'path', 'assesments', 'subject'));
            } else {
                return view('adminprimary/assesment_record/edit_lang_upper', compact('title', 'path', 'assesments', 'subject'));
            }

        }elseif($subject->subject == 'ART AND CRAFT') {
            return view('adminprimary/assesment_record/edit_art', compact('title', 'path', 'assesments', 'subject'));
        }elseif($subject->subject == 'MANDARIN') {
            return view('adminprimary/assesment_record/edit_mandarin', compact('title', 'path', 'assesments', 'subject'));
        } else {
            return view('adminprimary/assesment_record/edit', compact('title', 'path', 'assesments', 'subject'));
        }
    ...
    public function export($id_ass)
    {
        ...
        if($subject == 'HEALTH AND PHYSICAL EDUCATION') {
            ...
            return Excel::download(new AdminMandarinPrimaryAssesmentRecordExport($id), "Assesment Record - {$subject} - {$class}.xlsx");
        }else{
            return Excel::download(new AdminPrimaryAssesmentRecordExport($id_ass), "Assesment Record - {$subject} - {$class}.xlsx");
        }
